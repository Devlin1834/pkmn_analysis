...
drops remainder for lists that dont have lens evenly divided by n"""
...
class Table():
    ...
    ###########################################################################
    """outputs a flattened list of raw data"""
    def flatten(self, by = 'row'):
        out = {'row': [self.raw_data[j][h] for h in range(3) for j in range(3)],
               'col': [self.raw_data[h][j] for h in range(3) for j in range(3)]}
        
        return out[by]
    
    ###########################################################################
    """Returns a given row or column, based on their names or numbers (starting at 1)"""
    def pull(self, given, ask = 'col'):
        contents = self.get_cells()
        if type(given) is int:
            found = {'col': [c.data for c in contents if c.x == given - 1],
                     'row': [c.data for c in contents if c.y == given - 1]}
        else:
            sbjct = given.lower()
            found = {'col': [c.data for c in contents if c.col.lower() == sbjct],
    ...
    Passing ints lets you select rows and cols by number - starting at 1"""
    ...
    def row_sort(self, sbjct):
        ...
        if type(sbjct) is str and sbjct.lower() in rlow:
            sbjct = rlow.index(sbjct.lower()) + 1
        elif type(sbjct) is str and sbjct.lower() == 'names':
            sbjct = 0
        
        ## Step 1: Set up the basis for sorting
        cells = list_break(self.get_cells(), self.nrows)
        if sbjct > 0:
            source = cells.copy()
            cells.sort(key = lambda x: str(x[sbjct - 1].data))
            places = {source.index(x): cells.index(x) for x in source}
        else:
            source = self.cnames.copy()
            self.cnames.sort()
            places = {source.index(x): self.cnames.index(x) for x in source}
         
        ## Step 2: Sort
        empty = ['' for i in range(self.ncol)]
        for c in range(self.ncol):
            x = self.cnames if sbjct > 0 else cells
            n = x[c]
            new = places[c]
            empty[new] = n
        
        ## Step 3: Set up the Reset
        if sbjct > 0:
            self.cnames = empty
            to_reset = cells
        else:
        ...
        ## Step 4: Reset and strip the cell class from the data
        reset = []
        for lst in to_reset:
            new = [c.data for c in lst]
            reset.append(new)
    ...
    def generate_table(self):
        ...
        if self.print_summary:
            ...
            for i in range(self.ncol):
                col = self.summary[i]
                blanks = self.widths[i] - len(col)
                name_box = b + (s * blanks) + col + s
...
if __name__ == '__main__':
    ...
    print("f.pull(2, 'col')  >> {}".format(f.pull(2, 'col')))
    # f.pull(2, 'col') >> [6, '5', 4]
    ...
    print("f.point(2, 3)      >> {}".format(f.point(2, 3)))
    # f.point(2, 3)     >> 4
    
    print("f.point('a', 'o')  >> {}".format(f.point('a', 'o')))
    # f.point('a', 'o') >> 3
    
    print("f.point(2, 'n')    >> {}".format(f.point(2, 'n')))
    # f.point(2, 'n')   >> 5
    ...
    print("\nSort by Rownames or Colnames with index 0 or 'names'\nf.col_sort('names')")
    f.col_sort('names')
    print(f)
    # ---------------------
    # |    |  a |  b |  c |
    ...
    print("""
When we pull points or rows/cols we start from 1 because of the sort funcs.
0 in a sort func refers to the row/col names, which are easily pulled using 
their respective attributes. So the data array starts at 1 to avoid confsion 
and be consistent. Entering 0 into a pull func will return an IndexError.""")
    
    print("""
Tables work in Tkinter too. Use the .table attribute as the text for tk.Label
and set the font to Courrier and the table gets printed to a window

Example Code (after imports):
    
    e = [[x ** i for i in range(1, 11)] for x in range(1, 6)]
    c = list(range(1, 6))
    r = list(range(1, 11))
    check = tg.Table(r, c, e)  

    class Test(tk.Tk):
        def __init__(self):
            super().__init__()
            self.frame = tk.Frame(self.master)
            self.lab = tk.Label(self.frame, text = check.table, font = ('Courier', 12))
            self.lab.pack()
            self.frame.pack()
...
This produces a single window with the table printed inside it. It even works
when just calling check and not check.table, but that looks gross to me.""")
